
import { useEffect, useRef } from 'react';
import { Link } from 'react-router-dom';
import { Button } from "@/components/ui/button";
import { ArrowRight, ShieldCheck } from "lucide-react";
import { motion } from 'framer-motion';
import { fadeIn, staggerContainer, scaleUp, buttonHover } from '@/lib/animation';

const HeroSection = () => {
  const particlesRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    // Simple particle animation
    if (!particlesRef.current) return;
    
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d');
    
    if (!ctx) return;
    
    particlesRef.current.appendChild(canvas);
    
    const resizeCanvas = () => {
      if (particlesRef.current) {
        canvas.width = particlesRef.current.clientWidth;
        canvas.height = particlesRef.current.clientHeight;
      }
    };
    
    window.addEventListener('resize', resizeCanvas);
    resizeCanvas();

    const particles: {
      x: number;
      y: number;
      size: number;
      speedX: number;
      speedY: number;
      opacity: number;
    }[] = [];

    // Create particles
    for (let i = 0; i < 80; i++) {
      particles.push({
        x: Math.random() * canvas.width,
        y: Math.random() * canvas.height,
        size: Math.random() * 2 + 0.1,
        speedX: (Math.random() - 0.5) * 0.3,
        speedY: (Math.random() - 0.5) * 0.3,
        opacity: Math.random() * 0.5 + 0.1,
      });
    }

    const drawParticles = () => {
      if (!ctx) return;
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      
      particles.forEach(particle => {
        ctx.beginPath();
        ctx.arc(particle.x, particle.y, particle.size, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(168, 85, 247, ${particle.opacity})`;
        ctx.fill();
        
        // Update position
        particle.x += particle.speedX;
        particle.y += particle.speedY;
        
        // Wrap around edges
        if (particle.x < 0) particle.x = canvas.width;
        if (particle.y < 0) particle.y = canvas.height;
        if (particle.x > canvas.width) particle.x = 0;
        if (particle.y > canvas.height) particle.y = 0;
      });
      
      // Draw connections between close particles
      particles.forEach((particle, i) => {
        for (let j = i + 1; j < particles.length; j++) {
          const distance = Math.sqrt(
            Math.pow(particle.x - particles[j].x, 2) +
            Math.pow(particle.y - particles[j].y, 2)
          );
          
          if (distance < 100) {
            ctx.beginPath();
            ctx.strokeStyle = `rgba(168, 85, 247, ${0.2 - distance / 500})`;
            ctx.lineWidth = 0.5;
            ctx.moveTo(particle.x, particle.y);
            ctx.lineTo(particles[j].x, particles[j].y);
            ctx.stroke();
          }
        }
      });
      
      requestAnimationFrame(drawParticles);
    };
    
    drawParticles();
    
    return () => {
      window.removeEventListener('resize', resizeCanvas);
      if (particlesRef.current) {
        particlesRef.current.removeChild(canvas);
      }
    };
  }, []);

  return (
    <section className="relative min-h-[95vh] flex items-center overflow-hidden">
      <div ref={particlesRef} className="particles-bg" />
      
      {/* Gradient orbs */}
      <motion.div 
        className="absolute top-1/3 -left-20 w-96 h-96 bg-scryptex-purple/20 rounded-full blur-3xl"
        animate={{ 
          scale: [1, 1.2, 1],
          opacity: [0.2, 0.3, 0.2]
        }} 
        transition={{ 
          duration: 8,
          repeat: Infinity,
          repeatType: "reverse"
        }}
      />
      <motion.div 
        className="absolute bottom-1/4 -right-20 w-96 h-96 bg-scryptex-lightpurple/20 rounded-full blur-3xl"
        animate={{ 
          scale: [1, 1.1, 1],
          opacity: [0.2, 0.25, 0.2]
        }} 
        transition={{ 
          duration: 6,
          repeat: Infinity,
          repeatType: "reverse",
          delay: 1
        }}
      />
      
      <motion.div 
        variants={staggerContainer(0.2, 0.1)}
        initial="hidden"
        whileInView="show"
        viewport={{ once: true, amount: 0.25 }}
        className="container mx-auto px-4 py-20 grid grid-cols-1 md:grid-cols-2 items-center gap-12"
      >
        <div>
          <motion.div 
            variants={fadeIn('right', 0.2)} 
            className="flex items-center gap-2 mb-6"
          >
            <div className="w-8 h-8 bg-scryptex-purple/30 rounded-lg flex items-center justify-center">
              <ShieldCheck className="w-4 h-4 text-scryptex-lightpurple" />
            </div>
            <span className="text-sm font-medium text-scryptex-lightpurple">AI-POWERED SECURITY</span>
          </motion.div>
          
          <motion.h1 
            variants={fadeIn('right', 0.4)} 
            className="text-4xl md:text-5xl lg:text-7xl font-bold mb-6 leading-tight"
          >
            <span className="bg-gradient-to-r from-scryptex-purple via-scryptex-lightpurple to-white text-transparent bg-clip-text">
              Revolutionize Web3 Research
            </span> <br />with AI Intelligence
          </motion.h1>
          
          <motion.p 
            variants={fadeIn('right', 0.6)} 
            className="text-lg md:text-xl text-gray-300 mb-8 max-w-xl"
          >
            Scryptex empowers on-chain research, farming automation, and project insights using AI agents. Never miss a crucial signal again.
          </motion.p>
          
          <motion.div 
            variants={fadeIn('right', 0.8)} 
            className="flex flex-wrap gap-4"
          >
            <motion.div whileHover="hover" variants={buttonHover}>
              <Button 
                asChild
                size="lg" 
                className="bg-gradient-to-r from-scryptex-purple to-scryptex-lightpurple hover:opacity-90 transition-all text-white rounded-lg px-8 shadow-[0_0_15px_rgba(168,85,247,0.5)]"
              >
                <Link to="/dashboard" className="flex items-center gap-2">
                  Get Started <ArrowRight className="h-4 w-4" />
                </Link>
              </Button>
            </motion.div>
            
            <motion.div whileHover="hover" variants={buttonHover}>
              <Button 
                variant="outline"
                size="lg"
                className="text-white border-white/20 hover:bg-white/10 transition-all rounded-lg"
              >
                Learn More
              </Button>
            </motion.div>
          </motion.div>
          
          <motion.div 
            variants={fadeIn('right', 1)} 
            className="mt-12 flex items-center gap-6"
          >
            <div className="flex -space-x-3">
              <motion.div 
                whileHover={{ y: -5 }}
                transition={{ type: "spring", stiffness: 300 }}
                className="w-8 h-8 rounded-full bg-gradient-to-r from-blue-500 to-blue-600"
              ></motion.div>
              <motion.div 
                whileHover={{ y: -5 }}
                transition={{ type: "spring", stiffness: 300 }}
                className="w-8 h-8 rounded-full bg-gradient-to-r from-green-500 to-green-600"
              ></motion.div>
              <motion.div 
                whileHover={{ y: -5 }}
                transition={{ type: "spring", stiffness: 300 }}
                className="w-8 h-8 rounded-full bg-gradient-to-r from-yellow-500 to-yellow-600"
              ></motion.div>
              <motion.div 
                whileHover={{ y: -5 }}
                transition={{ type: "spring", stiffness: 300 }}
                className="w-8 h-8 rounded-full bg-gradient-to-r from-red-500 to-red-600"
              ></motion.div>
            </div>
            <span className="text-sm text-gray-400">Trusted by <span className="text-white font-medium">10,000+</span> researchers</span>
          </motion.div>
        </div>
        
        <motion.div 
          variants={scaleUp(0.6)}
          className="relative"
        >
          <div className="absolute inset-0 bg-gradient-to-r from-scryptex-purple/20 to-scryptex-lightpurple/20 rounded-lg blur-2xl"></div>
          <motion.div 
            className="w-full max-w-lg mx-auto rounded-lg overflow-hidden gradient-border"
            animate={{ 
              boxShadow: ["0 0 10px rgba(168, 85, 247, 0.3)", "0 0 30px rgba(168, 85, 247, 0.5)", "0 0 10px rgba(168, 85, 247, 0.3)"]
            }}
            transition={{
              duration: 4,
              repeat: Infinity,
              repeatType: "reverse"
            }}
          >
            <div className="bg-black/80 p-1">
              <div className="relative rounded-md overflow-hidden">
                {/* Mock UI Preview */}
                <div className="bg-black/80 rounded-md p-4 backdrop-blur">
                  <div className="h-96 w-full rounded flex items-center justify-center">
                    <div className="grid grid-cols-3 gap-4 w-full h-full p-4">
                      <div className="col-span-2 bg-black/60 rounded-lg p-4 flex flex-col">
                        <motion.div
                          animate={{ 
                            opacity: [0.6, 0.8, 0.6],
                            width: ["70%", "75%", "70%"]
                          }}
                          transition={{ 
                            duration: 3,
                            repeat: Infinity,
                            repeatType: "reverse"
                          }}
                          className="h-8 w-3/4 bg-scryptex-purple/20 rounded mb-4"
                        ></motion.div>
                        <div className="flex-1 grid grid-cols-2 gap-3">
                          <motion.div 
                            animate={{ opacity: [0.1, 0.2, 0.1] }}
                            transition={{ 
                              duration: 4,
                              repeat: Infinity,
                              repeatType: "reverse"
                            }}
                            className="bg-scryptex-purple/10 rounded"
                          ></motion.div>
                          <motion.div 
                            animate={{ opacity: [0.2, 0.3, 0.2] }}
                            transition={{ 
                              duration: 3.5,
                              repeat: Infinity,
                              repeatType: "reverse",
                              delay: 0.5
                            }}
                            className="bg-scryptex-purple/20 rounded"
                          ></motion.div>
                          <motion.div 
                            animate={{ opacity: [0.3, 0.4, 0.3] }}
                            transition={{ 
                              duration: 5,
                              repeat: Infinity,
                              repeatType: "reverse"
                            }}
                            className="bg-scryptex-purple/30 rounded"
                          ></motion.div>
                          <motion.div 
                            animate={{ opacity: [0.1, 0.15, 0.1] }}
                            transition={{ 
                              duration: 4,
                              repeat: Infinity,
                              repeatType: "reverse",
                              delay: 1
                            }}
                            className="bg-scryptex-purple/10 rounded"
                          ></motion.div>
                        </div>
                      </div>
                      <div className="col-span-1 flex flex-col gap-4">
                        <div className="h-1/2 bg-black/60 rounded-lg p-3">
                          <motion.div
                            animate={{ 
                              width: ["60%", "65%", "60%"]
                            }}
                            transition={{ 
                              duration: 3,
                              repeat: Infinity,
                              repeatType: "reverse"
                            }}
                            className="h-4 w-2/3 bg-scryptex-purple/20 rounded mb-3"
                          ></motion.div>
                          <motion.div
                            animate={{ 
                              opacity: [0.1, 0.2, 0.1]
                            }}
                            transition={{ 
                              duration: 4,
                              repeat: Infinity,
                              repeatType: "reverse"
                            }}
                            className="h-24 bg-scryptex-purple/10 rounded-lg"
                          ></motion.div>
                        </div>
                        <div className="h-1/2 bg-black/60 rounded-lg p-3">
                          <motion.div
                            animate={{ 
                              width: ["45%", "50%", "45%"]
                            }}
                            transition={{ 
                              duration: 3.5,
                              repeat: Infinity,
                              repeatType: "reverse"
                            }}
                            className="h-4 w-1/2 bg-scryptex-purple/20 rounded mb-3"
                          ></motion.div>
                          <motion.div
                            animate={{ 
                              opacity: [0.1, 0.15, 0.1]
                            }}
                            transition={{ duration: 2, repeat: Infinity, repeatType: "reverse", delay: 0.2 }}
                            className="h-4 w-full bg-scryptex-purple/10 rounded mb-2"
                          ></motion.div>
                          <motion.div
                            animate={{ 
                              opacity: [0.1, 0.15, 0.1]
                            }}
                            transition={{ duration: 2, repeat: Infinity, repeatType: "reverse", delay: 0.5 }}
                            className="h-4 w-full bg-scryptex-purple/10 rounded mb-2"
                          ></motion.div>
                          <motion.div
                            animate={{ 
                              opacity: [0.1, 0.15, 0.1]
                            }}
                            transition={{ duration: 2, repeat: Infinity, repeatType: "reverse", delay: 0.8 }}
                            className="h-4 w-full bg-scryptex-purple/10 rounded mb-2"
                          ></motion.div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </motion.div>
        </motion.div>
      </motion.div>
    </section>
  );
};

export default HeroSection;
